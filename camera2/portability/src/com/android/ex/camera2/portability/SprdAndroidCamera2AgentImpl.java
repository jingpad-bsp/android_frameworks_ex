...
//    private boolean mNeedAfBeforeCapture = true;
    ...
    public static class LightPortraitCallbackForward implements CameraLightPortraitCallback {
        private final Handler mHandler;
        private final CameraLightPortraitCallback mCallback;
        private final CameraAgent.CameraProxy mCamera;

        public static LightPortraitCallbackForward getNewInstance(Handler handler,
                                                                  CameraProxy camera, CameraLightPortraitCallback cb) {
            if (handler == null || camera == null || cb == null)
                return null;
            return new LightPortraitCallbackForward(handler, camera, cb);
        }

        private LightPortraitCallbackForward(Handler h, CameraAgent.CameraProxy camera,
                                             CameraLightPortraitCallback cb) {
            mHandler = h;
            mCamera = camera;
            mCallback = cb;
        }
        ...
        @Override
        public void onLightPortraitStatusChange(final int status){
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    mCallback.onLightPortraitStatusChange(status);
                }
            });
        }
    }
    ...
    public void monitorLightPortraitStatus(CaptureResult result,
                                            AndroidCamera2ProxyImpl camera2Proxy) {
        Integer lightPortraitStatus = result.get(CaptureResult.ANDROID_SPRD_Light_PORTRAIT_TIPS);
...
*/
    ...
    public class SprdCamera2Handler extends AndroidCamera2AgentImpl.Camera2Handler {
        ...
        private LightPortraitCallbackForward mLightPortraitCallback;
        ...
        public void setLightPortraitListener(LightPortraitCallbackForward listener) {
            mLightPortraitCallback = listener;
        }

        public LightPortraitCallbackForward getLightPortraitListener(){
            return mLightPortraitCallback;
        }
        ...
        public void handleMessage(final Message msg) {
            ...
            try {
                switch (cameraAction) {
                    ...
                    case CameraActions.SET_LIGHT_PORTRAIT_CALLBACK: {
                        setLightPortraitListener((LightPortraitCallbackForward)msg.obj);
                    }
        ...
        protected void setVideoRecoderSurface(Surface mediaSurface) {
            ...
            if(mCameraProxy.getCameraId() != SPRD_BACK_PORTRAIT_BACKGROUND_REPLACEMENT_ID && mCameraProxy.getCameraId() != SPRD_FRONT_PORTRAIT_BACKGROUND_REPLACEMENT_ID) {
        ...
        protected void setVideoRecoderSurfaceTexture(Surface mediaSurface) {
            ...
            try {
                if(mCameraProxy.getCameraId() != SPRD_BACK_PORTRAIT_BACKGROUND_REPLACEMENT_ID && mCameraProxy.getCameraId() != SPRD_FRONT_PORTRAIT_BACKGROUND_REPLACEMENT_ID) {
                    ...
                    mCamera.createCaptureSession(
                            Arrays.asList(mPreviewSurface, mMediaRecoderSurface),
                            mCameraPreviewStateCallback, this);
                }
            } catch (CameraAccessException ex) {
                Log.e(TAG, "Failed to create camera capture session", ex);
...
//            outConfigurations.add(new OutputConfiguration(mPreviewSurface, mPreviewSize.width(), mPreviewSize.height()));
    ...
    protected class SprdAndroidCamera2ProxyImpl extends
        ...
        @Override
        public void setLightPortraitCallback(Handler handler, CameraLightPortraitCallback callback) {
            mCameraHandler.obtainMessage(
                    CameraActions.SET_LIGHT_PORTRAIT_CALLBACK,
                    LightPortraitCallbackForward.getNewInstance(handler,
                            SprdAndroidCamera2ProxyImpl.this, callback)).sendToTarget();
        }
