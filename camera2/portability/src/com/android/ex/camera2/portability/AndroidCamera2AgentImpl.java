            ...
            try {
                ...
                case CameraActions.RECONNECT: {
                    ...
                    if (mCameraIndex == SPRD_3D_VIDEO_ID || mCameraIndex == SPRD_RANGE_FINDER_ID || mCameraIndex == SPRD_3D_CAPTURE_ID || mCameraIndex == SPRD_BLUR_ID
                            ...
                            || mCameraIndex == SPRD_BACK_HIRES_ID || mCameraIndex == SPRD_FRONT_HIRES_ID || mCameraIndex == SPRD_BACK_IR_ID
                            || mCameraIndex == SPRD_BACK_MACRO_ID || mCameraIndex == BACK_WT_FUSION_ID || mCameraIndex == SPRD_FRONT_PORTRAIT_ID
                            || mCameraIndex == SPRD_BACK_PORTRAIT_BACKGROUND_REPLACEMENT_ID || mCameraIndex == SPRD_FRONT_PORTRAIT_BACKGROUND_REPLACEMENT_ID) {
            ...
            for (int id = 0; id < cameraIds.length; ++id) {
                try {
                    ...
                    if (firstBackId == NO_DEVICE &&
                        ...
                        firstBackId = Integer.parseInt(cameraIds[id]);
                        // SPRD
                        super.updateFeatureEnable(cameraManager, cameraIds[id]);
                    }
                    if (firstFrontId == NO_DEVICE &&
                            lensDirection == CameraCharacteristics.LENS_FACING_FRONT) {
                        firstFrontId = Integer.parseInt(cameraIds[id]);
                    }
                } catch (CameraAccessException ex) {
